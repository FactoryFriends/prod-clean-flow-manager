...
export const Production = React.memo(function Production({ currentLocation }: ProductionProps) {
  ...
  const handlePrintLabels = useCallback((batch: ProductionBatch) => {
    setSelectedBatch(batch);
    setLabelDialogOpen(true);
  }, []);

  const handleEditBatch = useCallback((batch: ProductionBatch) => {
    setEditBatch(batch);
    setEditDialogOpen(true);
  }, []);

  const getExpiryStatus = useCallback((batch: ProductionBatch) => {
    const now = new Date();
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage production batches at {locationName}
            </p>
          </div>
        </div>
        <Card>
